...
If Not FileExists("lib\ImageSearchDLLx32.dll") Then FileInstall("lib\ImageSearchDLLx32.dll", "lib\ImageSearchDLLx32.dll", 1);FileInstall ( "source", "dest" [, flag = 0] )
If Not FileExists("lib\ImageSearchDLLx64.dll") Then FileInstall("lib\ImageSearchDLLx64.dll", "lib\ImageSearchDLLx64.dll", 1)
If Not FileExists("lib\msvcr110d.dll") Then FileInstall("lib\msvcr110d.dll", "lib\msvcr110d.dll", 1);Microsoft Visual C++ Redistributable dll x64
If Not FileExists("lib\msvcr110.dll") Then FileInstall("lib\msvcr110.dll", "lib\msvcr110.dll", 1);Microsoft Visual C++ Redistributable dll x32
#EndRegion
...
Func _ImageSearchStartup()
	...
	If $sOSArch = "X86" Or $sAutoItX64 = 0 Then
		...
		$h_ImageSearchDLL = DllOpen("lib\ImageSearchDLLx32.dll")
	...
	ElseIf $sOSArch = "X64" And $sAutoItX64 = 1 Then
		...
		$h_ImageSearchDLL = DllOpen("lib\ImageSearchDLLx64.dll")
		If $h_ImageSearchDLL = -1 Then Return "DllOpen failure"
	Else
...
Func _ImageSearch($findImage, $resultPosition, ByRef $x, ByRef $y, $tolerance, $transparency = 0)
	Return _ImageSearchArea($findImage, $resultPosition, 0, 0, $res_width, $res_height, $x, $y, $tolerance, $transparency)
EndFunc   ;==>_ImageSearch

Func _ImageSearchArea($findImage, $resultPosition, $x1, $y1, $right, $bottom, ByRef $x, ByRef $y, $tolerance = 0, $transparency = 0, $HBMP = 0);Credits to Sven for the Transparency addition
    ;MsgBox(0, "Alert", "2015")
	...
	;ConsoleWrite("HBMP = " & $HBMP & " DLL=" & $h_ImageSearchDLL & @CRLF)
	If $HBMP = 0 Then
		;ConsoleWrite("NO HBMP!")
		$result = DllCall($h_ImageSearchDLL, "str", "ImageSearch", "int", $x1, "int", $y1, "int", $right, "int", $bottom, "str", $findImage)
	Else
		;ConsoleWrite("HAS HBMP!")
		$result = DllCall($h_ImageSearchDLL, "str", "ImageSearchEx", "int", $x1, "int", $y1, "int", $right, "int", $bottom, "str", $findImage, "ptr", $HBMP)
	EndIf
